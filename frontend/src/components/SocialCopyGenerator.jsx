...
export default function SocialCopyGenerator() {
  ...
  // 附加控制
  const [cta, setCta] = useState("點擊收藏，加入綠生活 ??");
  ...
  return (
    <div style={S.page}>
      <div style={S.container}>
        ...
        <div style={S.sub}>OpenAI / Gemini（使用者自帶 API Key）。綠金米配色走簡潔高雅：內建深綠按鈕與淺色背景。</div>
        ...
        <div style={S.grid}>
          <div style={S.col}>
            <div style={S.card}>
              ...
              <input value={tone} onChange={(e)=>setTone(e.target.value)} style={S.input} placeholder="例：溫暖、專業、優雅"/>

              <div style={S.label}>目標（主/次）</div>
              <input value={goal} onChange={(e)=>setGoal(e.target.value)} style={S.input} placeholder="例：提升收藏/互動、導流官網"/>
              ...
              <div style={S.row}>
                ...
                <div>
                  ...
                  <input value={length} onChange={(e)=>setLength(e.target.value)} style={S.input} placeholder="短/中/長 或字數範圍"/>
                </div>
              </div>
            </div>
          ...
          <div style={S.col}>
            ...
            <div style={S.card}>
              ...
              <div style={{ display:"flex", gap:10, marginTop:10 }}>
                <button onClick={doPreview} style={S.btnGhost}>預覽 Prompt</button>
                <button onClick={()=>copyToClipboard(builtPrompt)} style={S.btnGhost}>複製 Prompt</button>
              </div>
              {preview && (
        ...
        <div style={{ display:"flex", gap:12, marginTop:16 }}>
          ...
          <button onClick={()=>copyToClipboard(output)} disabled={!output} style={{...S.btnGhost, opacity: output?1:0.6}}>複製輸出</button>
        </div>
